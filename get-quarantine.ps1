...
function Get-AllQuarantine {
    ...
    $quarantine = Get-QuarantineMessage -PageSize 1000 -StartReceivedDate (Get-Date).AddDays($i).ToString("MM/dd/yyyy 00:00:00") -EndReceivedDate (Get-Date).AddDays($i).ToString("MM/dd/yyyy 23:59:59") -Page $pagecount
